        ...
        if(resolution[1] <= 480){
            ...
            pipeline = (Pipeline) Gst.parseLaunch(
                    "uridecodebin uri=" + videoSourceUrl + " ! tee name=t "
                            ...
                            + "t. ! queue max-size-buffers=0 max-size-time=0 max-size-bytes=0 ! videoscale ! video/x-raw,width=1920,height=8000 ! x264enc bitrate=800 ! hlssink2 max-files=5 target-duration=10 name=high_sink location=" + playlistRoot.resolve("high_%05d.ts") + " playlist-location=" + playlistRoot.resolve("high_playlist.m3u8")
            );
        }
